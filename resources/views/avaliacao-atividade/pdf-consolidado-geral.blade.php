...
@foreach($dadosPorAtividade as $indexAtividade => $dados)
    @php
        $atividade = $dados['atividade'];
        $evento = $atividade->evento;
        $resumoPublico = $dados['resumoPublico'];
        $respostasPorPergunta = $dados['respostasPorPergunta'];
    @endphp

    <div class="sheet">
        <x-pdf.header
            title="Relatórios Consolidados"
            subtitle="Documento institucional consolidado · Filtro aplicado"
        >
            Consolidando relatórios do momento: <strong>{{ $atividade->descricao ?? 'Momento' }}</strong>
        </x-pdf.header>
