          ...
          <div class="row g-3">
            ...
            @role('administrador')
              <div class="col-md-6">
                ...
                <div class="form-text">Apenas administradores podem definir os papéis de acesso.</div>
              </div>
            @endrole
          </div>
        </div>
          ...
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <select name="identidade_genero" id="identidade_genero"
                ...
                @foreach(['Mulher Cisgênero', 'Mulher Transsexual', 'Homem Cisgênero', 'Homem Transsexual', 'Travesti', 'Não binárie', 'Prefiro não responder', 'Outro'] as $op)
                  <option value="{{ $op }}" {{ old('identidade_genero', $user->identidade_genero ?? '') == $op ? 'selected' : '' }}>{{ $op }}</option>
                @endforeach
              </select>
              @error('identidade_genero') <div class="invalid-feedback">{{ $message }}</div> @enderror
              <div id="ig_outro_wrap" class="mt-2" style="display:{{ old('identidade_genero', $user->identidade_genero ?? '') == 'Outro' ? 'block' : 'none' }}">
                ...
                @error('identidade_genero_outro') <div class="invalid-feedback">{{ $message }}</div> @enderror
              </div>
            </div>

            <div class="col-md-6">
              <label for="raca_cor" class="form-label">Raça / Cor</label>
              <select name="raca_cor" id="raca_cor" class="form-select @error('raca_cor') is-invalid @enderror">
                <option value="" disabled selected>Selecione...</option>
                @foreach(['Preta', 'Parda', 'Branca', 'Amarela', 'Indígena', 'Prefere não declarar'] as $op)
                  <option value="{{ $op }}" {{ old('raca_cor', $user->raca_cor ?? '') == $op ? 'selected' : '' }}>{{ $op }}</option>
                @endforeach
              </select>
            ...
            <div class="col-md-6">
              ...
              <select name="comunidade_tradicional" id="comunidade_tradicional"
                ...
                <option value="" disabled selected>Selecione...</option>
                @foreach(['Não', 'Povos indígenas', 'Comunidades Quilombolas', 'Povos Ciganos', 'Ribeirinhos', 'Extrativistas', 'Outro'] as $op)
                  <option value="{{ $op }}" {{ old('comunidade_tradicional', $user->comunidade_tradicional ?? '') == $op ? 'selected' : '' }}>{{ $op }}</option>
                @endforeach
              </select>
              @error('comunidade_tradicional') <div class="invalid-feedback">{{ $message }}</div> @enderror
              <div id="ct_outro_wrap" class="mt-2" style="display:{{ old('comunidade_tradicional', $user->comunidade_tradicional ?? '') == 'Outro' ? 'block' : 'none' }}">
                ...
                @error('comunidade_tradicional_outro') <div class="invalid-feedback">{{ $message }}</div> @enderror
              </div>
            </div>

            <div class="col-md-6">
              <label for="faixa_etaria" class="form-label">Faixa Etária</label>
              <select name="faixa_etaria" id="faixa_etaria" class="form-select @error('faixa_etaria') is-invalid @enderror">
                <option value="" disabled selected>Selecione...</option>
                @foreach(['Primeira infância (0 a 6 anos)', 'Criança (7 a 11 anos)', 'Adolescente (12 a 17 anos)', 'Adulto (18 a 59 anos)', 'Idoso (a partir dos 60 anos)'] as $op)
                  <option value="{{ $op }}" {{ old('faixa_etaria', $user->faixa_etaria ?? '') == $op ? 'selected' : '' }}>{{ $op }}</option>
                @endforeach
              </select>
              @error('faixa_etaria') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            <div class="col-md-6">
              <label for="pcd" class="form-label">Pessoa com Deficiência (PcD)</label>
              <select name="pcd" id="pcd" class="form-select @error('pcd') is-invalid @enderror">
                <option value="" disabled selected>Selecione...</option>
                @foreach(['Não', 'Física', 'Auditiva', 'Visual', 'Intelectual', 'Múltipla'] as $op)
                  <option value="{{ $op }}" {{ old('pcd', $user->pcd ?? '') == $op ? 'selected' : '' }}>{{ $op }}</option>
                @endforeach
              </select>
              @error('pcd') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="col-md-6">
              ...
              <select name="orientacao_sexual" id="orientacao_sexual"
                ...
                <option value="" disabled selected>Selecione...</option>
                @foreach(['Lésbica', 'Gay', 'Bissexual', 'Heterossexual', 'Prefere não declarar', 'Outra'] as $op)
                  <option value="{{ $op }}" {{ old('orientacao_sexual', $user->orientacao_sexual ?? '') == $op ? 'selected' : '' }}>{{ $op }}</option>
                @endforeach
              </select>
              @error('orientacao_sexual') <div class="invalid-feedback">{{ $message }}</div> @enderror
              <div id="os_outra_wrap" class="mt-2" style="display:{{ old('orientacao_sexual', $user->orientacao_sexual ?? '') == 'Outra' ? 'block' : 'none' }}">
                ...
                @error('orientacao_sexual_outra') <div class="invalid-feedback">{{ $message }}</div> @enderror
              </div>
            </div>
          </div>
        </div>
